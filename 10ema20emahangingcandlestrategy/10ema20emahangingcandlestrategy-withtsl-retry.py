...
def parse_args():
    parser = argparse.ArgumentParser(description='10/20EMA Swing Strategy Backtester')
...
def _find_entry(df, idx, debug_mode=False):
    ...
    date = candle['Date'][:10]
    if debug_mode:
        logs.append(f"DAY: {date} | Open:{o} High:{h} Low:{l} Close:{c} | 10EMA:{ema10:.2f} 20EMA:{ema20:.2f}")
    is_long = (l > ema10 and h > ema10 and ema10 > ema20)
    is_short = (h < ema10 and l < ema10 and ema10 < ema20)
    if debug_mode:
        logs.append(f"[Check] Candle fully above 10EMA: {'YES' if (l > ema10 and h > ema10) else 'NO'}")
        logs.append(f"[Check] Candle fully below 10EMA: {'YES' if (h < ema10 and l < ema10) else 'NO'}")
        logs.append(f"[Check] 10EMA > 20EMA: {'YES' if ema10 > ema20 else 'NO'}, 10EMA < 20EMA: {'YES' if ema10 < ema20 else 'NO'}")
...
def summarize_trades(trades):
    ...
    for y, group in df.groupby('year'):
        ...
        y_max_dd = y_drawdowns.max() if not drawdowns.empty else 0.0
        yearly_summary[y] = {
